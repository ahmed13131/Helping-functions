...
def hyperimputer(df,metric = "Linear Regression"): # there is also "Random Forests"
    """
...
def treelabel(df,columns=None):
    """
    df -> dataframe
    
    frequency labeling function desiqned for tree models
    
    """
    if columns != None and type(columns) == list:
        for i in columns:
            if len(df[i].unique()) < 10:
                freq = (df[i].value_counts() /len(df))
                d={}
...
def linearlabel(df,columns=None):
    """
    df -> dataframe
    columns -> list of columns to preform one hot encodings 
    for linear models
